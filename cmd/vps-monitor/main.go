...
func main() {
	...
	collector := monitor.NewCollector(cfg, submitter, logger)
	prober := monitor.NewProber(cfg, submitter, logger)
	...
	webServer, err := web.New(cfg, sqliteStore, clusterManager, notifiers, logger)
	if err != nil {
